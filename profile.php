...
<body >
  ...
  background-color: rgba(255,255,255,0.79); border-radius:10px; box-shadow:0 2px 5px rgba(0,0,0,0.5)">
    <div class="row">
      <div class="col-md-4">
        <img src="image.jpg" alt="User Picture" class="img-fluid rounded-circle ">
      </div>
      <div class="col-md-8">
        <h1 class="display-3"><?= ucfirst($_SESSION['patient']->first_name) . " ". ucfirst($_SESSION['patient']->last_name) ?> </h1>
        <p>Email: <?= $_SESSION['patient']->email ?></p>
        <p>Phone: <?= $_SESSION['patient']->phone ?></p>
      </div>
    </div>
  </div>
